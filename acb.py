...
def openLangMenu():
	...
	langButton = ttk.Button(langMenu, text="Apply changes", command=lambda: applyLang()).pack(side=tk.BOTTOM, pady=5)
...
#root.configure(background="#89b0b3", relief="solid")
...
if mapStatus["Alhambra"] == True:
	buttonAlhambra.config(image=onIMG)
else:
	buttonAlhambra.config(image=offIMG)

fixSH = tk.Button(frame2, text="Apply Changes", bg="#38393F").pack()
